...
vim.api.nvim_create_autocmd({ "FileType" }, {
	...
	callback = function()
		...
		vim.keymap.set("n", "<Leader>ec", ":call SlimuxSendCommand(expand('<cword>') . '.columns')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>ed", ":call SlimuxSendCommand('%debug')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>eh", ":call SlimuxSendCommand(expand('<cword>') . '.head()')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>ei", ":call SlimuxSendCommand(expand('<cword>') . '.info()')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>ek", ":call SlimuxSendCommand(expand('<cword>') . '.keys()')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>el", ":call SlimuxSendCommand('len(' . expand('<cword>') . ')')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>en", ":call SlimuxSendCommand('next(iter(' . expand('<cword>') . '))')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>ep", ":call SlimuxSendCommand(getline('.') . 'pass')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>eP", [[:call SlimuxSendCommand(getline('.') . 'print("hi")')<CR>]], {buffer=true})
		vim.keymap.set("n", "<Leader>er", ":call SlimuxSendCommand('print(' . expand('<cword>') . ')')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>eR", ":call SlimuxSendCommand('%run ' . expand('%'))<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>es", ":call SlimuxSendCommand(expand('<cword>') . '.size()')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>e1", ":call SlimuxSendCommand(expand('<cword>') . '[0]')<CR>", {buffer=true})
		vim.keymap.set("n", "<Leader>e2", ":call SlimuxSendCommand(expand('<cword>') . '.iloc[0]')<CR>", {buffer=true})
		...
		vim.keymap.set("n", "<Leader>ea", [[/if __name__ == "__main__":<CR> <bar> kVgg :SlimuxREPLSendSelection<CR>]], {buffer=true})
		vim.keymap.set(
			"n",
			"<Leader>ev",
